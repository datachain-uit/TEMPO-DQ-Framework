# Mean imputation of missing engagement features
#
# Fills missing values in the course-engagement feature table by replacing
# each missing numeric entry with that column's mean, computed over the
...
OUTPUT_PATH = "course_features_mean.csv"
...
def impute_dataframe(df, numeric_cols):
    """Impute ``numeric_cols`` in ``df`` with per-column means (in place)."""
